...
![ContextCheck: classic models, LLM assessments, and a test macro F1 of 0.725 for Logistic Regression](images/contextcheck_cover.png)

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/fatimashafqat-data/ContextCheck/blob/main/notebooks/contextcheck_toxicity_detector.ipynb)
[![Tests](https://github.com/fatimashafqat-data/ContextCheck/actions/workflows/tests.yml/badge.svg)](https://github.com/fatimashafqat-data/ContextCheck/actions/workflows/tests.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)

**Logistic Regression achieved 0.725 macro F1**, compared with 0.688 for Random Forest and 0.291 for a majority baseline on 3,682 held-out tweets. ContextCheck investigates where word-based classifiers and LLM assessments disagree, combining reproducible evaluation, six error case studies, feature interpretation, and a Colab demo. The LLM comparison is in progress: **Gemini 9/30; OpenRouter 29/30**.
...
![Recorded Gemini and Nemotron assessments for source 18879, contrasting a humorous reading with an offensive-language label](images/recorded_disagreement.png)
...
9 examples currently have answers from both LLMs; 1 of those pairs disagree. These are coverage counts, not a completed benchmark. Inspect the [paired records](results/cross_llm_disagreements.csv).
...
Exact two-sided McNemar testing gives **p = 0.000380**: Logistic Regression alone is correct on **174** test examples, and Random Forest alone on **113**. The observed accuracy difference is statistically significant at the 5% threshold under the independence assumption. This test concerns **accuracy**, not the macro-F1 gap, and does not establish performance on a different dataset.
...
|---|---|---:|---|
| Gemini | `gemini-3.8-flash` | 9/30 | Paused after HTTP 429 (quota or rate limit) |
| OpenRouter | `nvidia/nemotron-3-super-120b-a12b:free` | 29/30 | Provider HTTP 502; remaining response(s) pending |

**Full four-model benchmark scores are withheld until both sources reach 30/30.** Available responses remain inspectable, and failed requests remain missing.
...
1. Open [ContextCheck in Google Colab](https://colab.research.google.com/github/fatimashafqat-data/ContextCheck/blob/main/notebooks/contextcheck_toxicity_detector.ipynb) and click **Connect**.
